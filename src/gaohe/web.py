...
def _safe_error(value: object) -> str:
    ...
    if re.search(r"(?i)(api[_-]?key|token|secret|password|cookie|authorization)\s*[=:]", text):
        return "Source check failed; sensitive details hidden."
    return text
...
def _article_card(article: object) -> str:
    ...
    return f"<article><h3>{title}</h3><p class='meta'>{source} · {updated}</p><div class='article-text'>{render_article(body, findings)}</div></article>"
...
def _finding_row(finding: object) -> str:
    ...
    return f"<li><strong>{label}</strong> {_status_badge(_value(finding, 'evidence_status', 'pending'))}<br>{_text(_value(finding, 'summary', 'No summary'))}</li>"
...
def render_status_page(snapshot: Mapping[str, object] | Settings) -> str:
    ...
    runtime_rows = "".join(
        ...
        if not re.search(r"(?i)(api[_-]?key|token|secret|password|cookie|authorization)", str(label))
    )
